"""
build_cfrs.py
Builds the Campaign Finance Regulatory Score (CFRS) for Latin American &
Caribbean countries from the International IDEA Political Finance Database,
then saves a processed scores file.
...
RAW_PATH = ROOT / "data" / "raw" / "export_table_raw.csv"
OUT_PATH = ROOT / "data" / "processed" / "cfrs_scores.csv"

# Treat "No data" responses as absence of a rule (0) when True.
# Set to False to drop them instead, for the robustness comparison.
FILL_NODATA_AS_ZERO = True

# ── 1. Load raw IDEA export ──────────────────────────────────────────────────
df_raw = pd.read_csv(RAW_PATH, encoding="utf-8", on_bad_lines="skip", header=None)
...
def score_yesno(val):
    ...
    if v == "Yes":
        return 1.0
    if v in ("Yes, above certain threshold", "Sometimes"):
        return 0.5
    if v in ("No", "Not applicable", "nan"):
        return 0.0
    if v == "No data":
        return np.nan
    # Default: if it starts with "Yes", treat as partial
...
def score_oversight(val):
    """D1: independence of the oversight body."""
    v = str(val).strip()
    if v == "No data":
        return np.nan
    if v == "No institution specified":
        return 0.0
    # Truly independent specialised body
    if "Special agency for political finance" in v:
        return 1.0
    # Court, EMB, ministry, or auditing agency – partially independent
    if any(x in v for x in ("EMB", "Court", "Auditing agency", "Ministry")):
        return 0.5
    return 0.0


def score_sanctions(val):
    """D2: severity of sanctions."""
    v = str(val).strip()
    if v in ("Not applicable", "nan", ""):
        return 0.0
    # Strong: prison, deregistration, or loss of elected office
    if any(x in v for x in ("Prison", "Deregistration", "Loss of elected office")):
        return 1.0
    # Weak: only fines or loss of public funding
    if "Fines" in v or "Loss of public funding" in v:
        return 0.5
    return 0.0
...
# ── 5. Compute CFRS ──────────────────────────────────────────────────────────
if FILL_NODATA_AS_ZERO:
    # "No data" treated as absence of a rule (conservative).
    df[score_var_cols] = df[score_var_cols].fillna(0)
    df["CFRS"] = df[score_var_cols].sum(axis=1) / 12
else:
    # Drop "No data" from the denominator instead of scoring it 0.
    df["CFRS"] = df[score_var_cols].mean(axis=1, skipna=True)
...
# ── 7. Preview results ───────────────────────────────────────────────────────
preview = (
    df[["Country", "CFRS_scaled"] + score_var_cols]
    .sort_values("CFRS_scaled", ascending=False)
)
preview.columns = ["Country", "CFRS(0-100)"] + [c.replace("s_", "") for c in score_var_cols]
